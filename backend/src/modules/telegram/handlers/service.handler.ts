...
/** Handles `/service`: starts a technical service request. */
@Injectable()
export class ServiceCommandHandler implements CommandHandler {
  readonly command: BotCommandName = BotCommand.Service;

  constructor(private readonly responder: TelegramResponderService) {}

  async handle(context: HandlerContext): Promise<void> {
    await this.responder.sendText(context, BotMessage.service, Keyboards.backToMenu());
  }
}
